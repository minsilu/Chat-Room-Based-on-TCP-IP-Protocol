...
class DataBase(object):
    ...
    def delete_user(self,user_name):
        if user_name not in self.users["user_name"].values:
            ...
            return 0
        self.users.drop(self.users[self.users["user_name"]==user_name].index,inplace=True)
        self.friends.drop(self.friends[self.friends["user_name"]==user_name].index,inplace=True)
        self.friends.drop(self.friends[self.friends["friend_name"]==user_name].index,inplace=True)
        self.groups.drop(self.groups[self.groups["user_name"]==user_name].index,inplace=True)
        self.users.to_csv("database/user.csv",index=False)
    ...
    def add_friend(self,one,other):
        if other not in self.users["user_name"].values:
            print("user_name not exists")
            return 0
